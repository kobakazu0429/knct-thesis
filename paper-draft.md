...
また執筆者は呉工業高等専門学校 電気情報工学科に５年間在学し、情報、アルゴリズム、実験などでプログラミングをする機会の多さに対して、学生によって習得度合いが大きく違うことを感じた。その要因として、「授業方法や実習方法に問題がある場合」や「友達のコードを写し課題提出をこなし、理解していない場合」、「授業時間外でプログラミングをする機会が少ない場合」などが考えられた。これらから授業内容などが複雑化するほど「自分が何をしているか、何を作っているのかわからない」という問題が発生する。

## 研究方法と研究意義

本研究では、ブラウザ上で完結し軽量かつテスト可能な学習環境を提案する。

まずブラウザ上で完結することで「授業時間外でプログラミングをする機会が少ない」問題を解決できる。これは学習者が自主学習をするには PC に授業などと同じ学習環境を用意する必要があるが、一般に学習環境を用意するのは困難である。これに関してはのちに記載するアンケートの結果からもわかる。しかし今回構築したシステムはブラウザ上で完結しているため、学習者はブラウザのみ使えればよく、プログラミングを始める壁を低くすることできるため、自主学習を促すことへ繋がる。
...
そして本研究における C 言語のコンパイラである Clang についてである.
Clang は LLVM をフロントエンド、バックエンドとした C 言語のコンパイラであり、GCC(GNU Compiler Collection)の代替品を目標に開発されている.
LLVM を利用していることで本研究の要であるエディタ間の通信として Language Server Protocol の提供や formatting などを可能としている.
今回は先の研究にて述べたエラーの日本語化について模索、検証を行った。

まず Clang に置いてエラー内容は次のように表せる.

`def err_undeclared_var_use : Error<"use of undeclared identifier %0">;`
https://github.com/llvm/llvm-project/blob/llvmorg-15.0.7/clang/include/clang/Basic/DiagnosticSemaKinds.td#L5522

これは初学者にありがちな変数の定義をすることなく使用した際にコンパイル時に報告されるエラーだ。

この時`%0`には変数名が入る

該当ファイルを参照するとわかるが、clang に置いてえラー内容はハードコーディングされており国際化対応をするすべが現状用意されていない。

従って linux の sed コマンドにて

`sed -ie "s/use of undeclared identifier %0/変数 %0 は未定義です/g" "$CLANG_BUILD_DIR/src/clang/include/clang/Basic/DiagnosticSemaKinds.td"`
のようにした時
...
`use of undeclared identifier a`は`変数 a は未定義です`となる。

従って該当エラーを順次日本語化すればエラー内容の日本語化が可能であるが、英語と日本語の翻訳において語順の解決が難しく変数の順序を正しく設定することはできないとともに、実際に表示されるエラーは種類としてはそこまで多くないことがわかる.

従って、全てのエラーを日本語化するという方針からエラーを順次通知し、報告数順に翻訳することでコストパフォーマンスの最大化を図る方針とした。

現在このエラー報告システムの開発をおこなっている.

---

定量的評価について

先の研究ではアンケート調査による評価となっており、教育効果を示すにはより具体的な数値を出すための定量的な調査をおこなっていく必要がある。
補完機能による効率化を測るには従来のシステムと本システムにおいてタイプ数の変化やタイプ時間の計測が必要だ。次に補完機能の使用状況に関しては LSP の監視によって行える.

またその他の機能に関してはクリック数から算出できるだろう

これらの計測方法をより考える！

また学生ごとのアカウントを用意することで授業時間外での勉強時間などを計測できる.
