...
ブラウザ上で完結する学習環境に類するものに、「paiza.io(https://paiza.io/ja)」といったブラウザ上で完結する実行環境というものがあるが、ブラウザ上でプログラムを記述し、サーバー側でコンパイル、実行などを行い、結果のみを返却、ユーザーに表示するものにある。標準入力についてはコンパイル時にサーバー側に送る必要があり、インタラクティブではないため、ユーザーは全ての動作を実行前に知っておく必要があり、制約も多い。
...
- デザインが見やすかった．
- 授業用のエディタとして簡単でいいと思う。paiza より使いやすい。
- コードを共有しやすく、かなり使いやすかった
- 画期的だなと感じた．まず画面がわかりやすかった．そしてテスト機能や，誤字をした箇所をわかりやすく知らせてくれる機能が便利だった．また，普段学校にて使用するエディタなどは自宅で学校と同様の環境にするにはパソコンを普段から使用していない低学年の頃ではわからないことだらけで大変な記憶があったが，今回のシステムが自宅や寮などでも比較的簡単に使用できるようになるならとてもわかりやすいのでいいと思う．授業でこういったわかりやすいものを使いたい．
- とても便利でほとんどプログラムをしない自分でも使いやすかった。1,2 年の頃 Terapad を使用することにとても不満を覚えていたので、このシステムなら(勉強に意欲のある人なら)Terapad よりもとても快適に情報処理の学習に打ち込めると思う。
- エラー部分が分かりやすく、動作速度も速いので非常に使いやすかった。
- web 上で簡潔に完結しているのがすごいと思いました
- 授業では Cygwin を使用しているのですが、括弧やクオーテーションを 2 回入力しなくていいことがとても楽に感じました。またテストの機能で実際に使ってみたらこんな感じなのかということが実感でき、また失敗したときに何ができて何ができないのかということが場合分けされていたので間違いを分析しやすいと感じました。
- 自分が書いたプログラムがテスト機能により、正しいかどうかが即座にわかるので、C 言語を学ぶ際にとても効率が良いと感じました。
- 自動的に関数が出たり、｛｝の間に enter を押すと自動的に一行隙間ができるのが快適だった
- 学校で性能の高い PC への置き換えも難しく，また学生一人ひとりの経済的な格差も大きい中で，Web 上で動作するこのシステムは安価で性能の低い PC でも十分動作するものであり，遠隔授業を行う上でも非常に合理的だと感じた．

---

[本研究自体へ総括して意見や感想があれば教えてください]

- 呉高専のプログラミング習熟度合いが違うというのは痛いほど感じているので，頑張ってほしい
- このシステムのような、C 言語を効率よく学べる環境を作ってくださるのは、とてもありがたいです。
